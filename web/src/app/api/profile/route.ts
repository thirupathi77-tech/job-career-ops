...
function patchToProfile(p: ProfilePatch): Record<string, unknown> {
  ...
  if (p.preferredName) candidate.preferred_name = p.preferredName;
  if (p.pronouns) candidate.pronouns = p.pronouns;
  ...
  if (p.portfolio) candidate.portfolio = p.portfolio;
  if (p.location) candidate.location = p.location;
  if (p.address) candidate.address = p.address;
  if (p.streetAddress) candidate.street_address = p.streetAddress;
  if (p.addressLine2) candidate.address_line2 = p.addressLine2;
  if (p.postalCode) candidate.zip_code = p.postalCode;
  if (p.currentCompany) candidate.current_company = p.currentCompany;
  if (p.currentTitle) candidate.current_title = p.currentTitle;
  ...
  if (p.relocation) loc.relocation = p.relocation;
  if (p.jobSearchLocation) loc.job_search_location = p.jobSearchLocation;
  ...
  if (p.workStyle) out.work_style = p.workStyle;
