...
.co-src__chip[data-state="active"]{border-color:color-mix(in srgb,var(--color-brand) 48%,transparent)}
.co-src__orb{width:.55rem;height:.55rem;border-radius:50%;background:var(--color-brand);box-shadow:0 0 0 0 color-mix(in srgb,var(--color-brand) 50%,transparent);animation:co-orb 1.4s ease-out infinite}
.co-src__bar{height:3px;border-radius:2px;background:var(--color-brand);transition:width .4s ease}
      ...
      <div className="co-disc">
        ...
        <div>
          ...
          <p className="mt-2 text-xs text-faint">
            {filters.positive.length ? `Roles: ${filters.positive.join(", ")}` : "All role titles"}
            {` · posted within ${filters.sinceDays === 1 ? "24 hours" : `${filters.sinceDays} days`}`}
          </p>
        </div>

        <div className="co-src">
          {ATS_SOURCES.filter((a) => filters.ats.includes(a)).map((a) => (
